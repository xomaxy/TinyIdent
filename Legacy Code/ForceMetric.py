...
def NeoHookeanBead(delta, p, R=10e-6):
    """This function represents the relationship between the force and the
    indentation delta for a neo-hookean material. If adhesion should be
    considered an additional term has to be considered"""
    C = p * np.pi * np.sqrt(R)
...
class AFMScan(Wave):
    ...
    def PlaneSubtraction(self, data, direction='xy', xdim=20e-6, ydim=20e-6):
        ...
        px = np.array([np.polyfit(x, img[i], 1)
            for i in np.arange(dx)]).mean(axis=0)
        py = np.array([np.polyfit(y, img[:,i], 1)
    ...
    def ProjectOnHeight(self, data,
        ...
        s = mlab.mesh(X, Y, Z, scalars=data, colormap=cmap)
        mlab.scalarbar(title=zlabel)
        # mlab.orientation_axes()
        # mlab.outline(figure='Projection on height')
        # mlab.axes()
        if save:
            mlab.savefig(save)
        else:
...
class StaticYoung(ContactPoint):
    ...
    def Young(self, model='h', ind=None, f=None, p0=[1e6, 1e-9],
...
#        cdef int contactidx
    ...
    def plot(self, X='indentation', save=False, rc_params='auto'):
        sns.set_palette('Set1')
        if rc_params is 'auto':
            rc_params = {'font.size':22,
        ...
        ax1.plot(self.indentation.Trace()/micro,
                 self.force.Trace()/nano, label='Trace', lw=3)
        ax1.plot(self.indentation.Retrace()/micro,
                 self.force.Retrace()/nano, label='Retrace', lw=3)
...
class Multicurve(ForceCurve):
    ...
    def LoadCurves(self, no=False, trace=True):
        p = self.path
